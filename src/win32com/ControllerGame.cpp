...
/*		if ( !nCount ) {
		...
		case 0:
			if ( fOK ) {
				strcpy(szLine, "ROM set is good; VPinSPA is able to use it");
				fROMSetOK = true;
			}
			else if ( fMaybeOK ) {
				...
				strcpy(szLine, "There are CRC and/or length errors! VPinSPA may or may not successfully be able to use the ROM set.\r\nIf sound roms are missing, sound will be disabled.");
			}
			else
				strcpy(szLine, "ROM set is bad: VPinSPA can't use it.");
			break;

		case 1:
			strcpy(szLine, "No ROMs defined for this game: VPinSPA can't run this game.");
			break;

		case 2:
			strcpy(szLine, "ROM set is missing: VPinSPA can't run the game.");
			break;

		case 3:
			strcpy(szLine, "This game is not supported by VPinSPA.");
			break;
		}
