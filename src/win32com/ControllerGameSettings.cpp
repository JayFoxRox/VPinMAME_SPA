	...
	LRESULT OnOK(WORD, UINT, HWND, BOOL&) {
		if ( m_fChanged ) {
			...
			if ( IsEmulationRunning() )
				MessageBox("Your changes will not take effect until you restart Visual PinSPA!","Notice!",MB_OK | MB_ICONINFORMATION);
		}
...
STDMETHODIMP CGameSettings::put_Value(BSTR sName, VARIANT newVal)
{
	...
	if ( SUCCEEDED(hr) ) {
		if ( IsEmulationRunning() && SettingAffectsRunningGame(szName) ) {
			...
			PostMessage(win_video_window, RegisterWindowMessage(VPinSPAADJUSTWINDOWMSG), 0, 0);
		}
	}
