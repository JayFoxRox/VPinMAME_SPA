...
#define VPinSPAONEVENTMSG	"VPinSPAOnEventMsg"
...
// these two functions are called from within PinSPA
...
extern "C" void OnSolenoid(int nSolenoid, int IsActive)  {
	...
	if ( IsWindow(m_pController->m_hEventWnd) )
		PostMessage(m_pController->m_hEventWnd, RegisterWindowMessage(VPinSPAONEVENTMSG), EVENT_ONSOLENOID, (WPARAM) ((IsActive?0x10000:0x00000) + nSolenoid));
}
...
extern "C" void	OnStateChange(int nState)  {
	...
	// post the call to the event window (which is created in the thread of the host app)
	if ( IsWindow(m_pController->m_hEventWnd) )
		PostMessage(m_pController->m_hEventWnd, RegisterWindowMessage(VPinSPAONEVENTMSG), EVENT_ONSTATECHANGE, (WPARAM) nState);
}
...
DWORD FAR PASCAL CController::RunController(CController* pController)
{
	...
	if ( fIsSupported==VARIANT_FALSE ) {
		MessageBox(pController->m_hParentWnd, "This game is not supported by Visual PinSPA", "Failed to start", MB_ICONINFORMATION|MB_OK);
		return 1;
	}
...
{
	static int uVPinSPAOnEventMessage = 0;
	static CController* pController  = NULL;

	switch ( uMsg ) {
	case WM_CREATE:
		pController = (CController*) ((LPCREATESTRUCT) lParam)->lpCreateParams;
		uVPinSPAOnEventMessage = RegisterWindowMessage(VPinSPAONEVENTMSG);
		break;

	default:
		if ( uMsg==uVPinSPAOnEventMessage ) {
			switch ( wParam ) {
...
void CreateEventWindow(CController* pController)
{
	...
	if ( !fEventWindowClassCreated )
	{
		...
		wc.lpszClassName 	= "VPinSPAEVENTWND";
	...
	pController->m_hEventWnd = CreateWindow( "VPinSPAEVENTWND", "", 0, 0, 0, 0, 0, NULL, NULL, _Module.m_hInst, (LPVOID*) pController);
}
...
extern "C" LRESULT CALLBACK osd_hook(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam, BOOL *pfhandled)
{
	...
	static UINT uVPinSPAAdjustWindowMsg = 0;
	...
	case WM_CREATE:
		...
		uVPinSPAAdjustWindowMsg = RegisterWindowMessage(VPinSPAADJUSTWINDOWMSG);
	...
	if ( message==uVPinSPAAdjustWindowMsg ) {
		iWindowStyle = 0;
