...
int frontend_help (const char *gamename)
{
	...
	if (help)  /* brief help - useful to get current version info */
	{
		...
		printf("PinSPA v%s\n Pinball's Multiple Arcade Machine Emulator\n"
				"Copyright (C) 2000-2018 by the PinSPA Team\n\n",build_version);
		showdisclaimer();
		printf("Usage:  PinSPA gamename [options]\n\n"
...
#ifndef MESS
	#ifdef PINMAME
			printf("\nPinSPA currently supports the following games:\n\n");
	#else
...
#ifdef PINMAME
			printf("This is the complete list of games supported by PinSPA %s.\n",build_version);
#else
