...
void hc55516_update(int num, INT16 *buffer, int length)
{
 ...
 #define LOWPASS_ESTIMATE_CYCLES 666
	...
	if (length > SAMPLE_RATE/2048  //!! magic // PinSPA: be less conservative/more precise
		&& chip->last_sound != 0)  // clock did not update next_value since the last update -> fade to silence (resolves clicks and simulates real DAC kinda)
	{
		...
		chip->integrator = 0.; // PinSPA: Reset all chip state
		chip->filter = 0.;
		chip->shiftreg = 0;
	}
	else
	{
		...
		INT32 slope = (((INT32)chip->next_value - data) << 15) / length; // PinSPA: increase/fix precision issue!
		data <<= 15;
