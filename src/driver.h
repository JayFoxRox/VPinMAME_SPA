...
#if defined(VPINMAME_ALTSOUND) || defined(VPinSPA_PINSOUND)
  int sound_mode; // 0 = PinSPA, 1 = altsound, 2 = pinsound, 3 = pinsound + recordings
#endif
#ifdef PROC_SUPPORT
...
#define MDRV_DIPS(no) \
  machine->PinSPA.coreDips = (no);

#define SWITCH_UPDATE(name) \
  void name##_switch_update(int *inports)

#define MDRV_SWITCH_UPDATE(name) \
  machine->PinSPA.updSw = name##_switch_update;
#define MDRV_DIAGNOSTIC_LEDH(no) \
  machine->PinSPA.diagLEDs = (no);
#define MDRV_DIAGNOSTIC_LEDV(no) \
  machine->PinSPA.diagLEDs = (no)|DIAGLED_VERTICAL;
#define MDRV_DIAGNOSTIC_LED7 \
  machine->PinSPA.diagLEDs = 0xff;

#define MDRV_SWITCH_CONV(toMatrix,fromMatrix) \
  machine->PinSPA.sw2m = toMatrix; \
  machine->PinSPA.m2sw = fromMatrix;
#define MDRV_LAMP_CONV(toMatrix,fromMatrix) \
  machine->PinSPA.lamp2m = toMatrix; \
  machine->PinSPA.m2lamp = fromMatrix;
...
#define MDRV_CORE_INIT_RESET_STOP(iname, rname, sname) \
  machine->PinSPA.init = machine_init_##iname; \
  machine->PinSPA.reset = machine_reset_##rname; \
  machine->PinSPA.stop = machine_stop_##sname;
