...
int pindmdInit(const char* GameName, UINT64 HardwareGeneration, const tPMoptions *Options) {
	...
	// look for the DmdDevice(64).dll in the path of vpinspa.dll
	char filename[MAX_PATH];

#ifndef _WIN64
	const HINSTANCE hVpmDLL = GetModuleHandle("vpinspa.dll");
#else
	const HINSTANCE hVpmDLL = GetModuleHandle("VPinSPA64.dll");
#endif
