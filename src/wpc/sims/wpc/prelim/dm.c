  ...
  void dm_wpc_proc_solenoid_handler(int solNum, int enabled, int smoothed) {
    ...
    if (!smoothed) {
      ...
      if (solNum == 18 || solNum == 19) {
        if (enabled)
          motor_PinSPA |= (1 << solNum);
        else
          motor_PinSPA &= ~(1 << solNum);
        // ignore states where both motors are enabled
        if (motor_PinSPA != (CLAW_MOTOR_LEFT | CLAW_MOTOR_RIGHT)) {
          // motor_PinSPA and motor_proc are either _OFF, _LEFT or _RIGHT
          // Turn off the motor if it's _LEFT or _RIGHT, then turn it on a
          // direction if necessary.
          if (motor_PinSPA != motor_proc) {
            ...
            if (motor_PinSPA == CLAW_MOTOR_LEFT) {  // _OFF or _RIGHT to _LEFT
              default_wpc_proc_solenoid_handler(18, TRUE, TRUE);
            }
            if (motor_PinSPA == CLAW_MOTOR_RIGHT) { // _OFF or _LEFT to _RIGHT
              default_wpc_proc_solenoid_handler(19, TRUE, TRUE);
            }

            motor_proc = motor_PinSPA;
          }
        }
      }
      return;
