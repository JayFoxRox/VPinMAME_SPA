...
#include "snd_cmd.h"
 ...
 //#define VPinSPA_PINSOUND // pmoptions.sound_mode == 2 || 3
#endif
...
#ifndef _WIN64
	const HINSTANCE hInst = GetModuleHandle("vpinspa.dll");
#else
	const HINSTANCE hInst = GetModuleHandle("VPinSPA64.dll");
#endif
...
{
	const LPTSTR PinSPASlotName = TEXT("\\\\.\\mailslot\\PinSoundPinSPA");
	...
	hFilePinSPA = makeReadSlot(PinSPASlotName);
	...
	response = readSlot(hFilePinSPA, buffer_msg);
	if(response != TRUE)
	{
		pinsound_studio_enabled = FALSE;
		CloseHandle(hFilePinSound);
	...
	else
	{
		...
		if(responseValue == 1)
		{
			...
			// force internal PinSPA volume mixer to 0 to mute emulated sounds & musics
