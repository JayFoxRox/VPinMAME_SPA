// InstallVPinSPA.cpp : Defines the entry point for the application.
//
#define _WIN32_DCOM
...
#ifndef _WIN64
 #define VPMDLLNAME "vpinspa.dll"
#else
 #define VPMDLLNAME "VPinSPA64.dll"
#endif

int RegisterUnregisterVPinSPA(HWND hWnd, int fRegister)
{
	HRESULT hr;
	HMODULE hModule;

	// so now let's see if we can load vpinspa.dll
	...
	char szMsg[256];
	if ( fRegister )
		wsprintf(szMsg, "You have selected to install version %s of Visual PinSPA! \nAre you ready to proceed?", szVersion);
	else
		wsprintf(szMsg, "You have selected to uninstall version %s of Visual PinSPA! \nAre you ready to proceed?", szVersion);	
	...
	if ( MessageBox(hWnd, szMsg, g_szCaption, MB_ICONQUESTION|MB_YESNO)==IDNO ) {
		if ( fRegister )
			...
			MessageBox(hWnd, "UnInstallation aborted!\nVisual PinSPA is still installed on your system", g_szCaption, MB_ICONEXCLAMATION|MB_OK);
		FreeLibrary(hModule);
		return 0;
	}
	...
	if ( fRegister ) {
		...
		if ( !DllRegisterServer ) {
			DisplayError(hWnd, GetLastError(), "Can't find the registering function (DllRegisterServer) in the " VPMDLLNAME ".", "Please check if you have a valid vpinspa.dll!");
			FreeLibrary(hModule);
			return 0;
		}
		...
		hr = CLSIDFromProgID(OLESTR("VPinSPA.Controller"), &ClsID);
		if ( FAILED(hr) ) {
		...
		if ( !DllUnregisterServer ) {
			DisplayError(hWnd, GetLastError(), "Can't find the unregistering function (DllUnegisterServer) in the " VPMDLLNAME ".", "Please check if you have a valid vpinspa.dll!");
			FreeLibrary(hModule);
			return 0;
		}
	...
	if ( !fRegister ) {
		if ( MessageBox(hWnd, "Do you want also delete all registry entrys Visual PinSPA has made?", g_szCaption, MB_ICONQUESTION|MB_YESNO)==IDYES ) 
...
int DisplayDialogs(HWND hWnd, int nDialog)
{
	...
	hr = CoCreateInstance(ClsID, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void**) &pUnknown);
	if ( FAILED(hr) ) {
		DisplayError(hWnd, hr, "Can't create the Controller class! \nPlease check that you have installed Visual PinSPA properly!");
		return 0;
	}
	...
	if ( FAILED(hr) ) {
		if ( hr==DISP_E_EXCEPTION )
			...
			MessageBoxW(0, ExcepInfo.bstrDescription, L"Error from the VPinSPA.Controller interface", MB_ICONERROR|MB_OK);
		else 
			DisplayError(hWnd, hr, "Can't get the dispatch interface");
...
void DisplayInstalledVersion(HWND hWnd)
{
	...
	if(strlen(szInstalledVersion) > 0)
		wsprintf(szVersionText, "* Visual PinSPA Version %s is currently installed on your computer *", szInstalledVersion);
	else
		wsprintf(szVersionText, "* Visual PinSPA is not currently installed on your computer *");
...
INT_PTR PASCAL MainDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_REGISTER:
			if ( RegisterUnregisterVPinSPA(hWnd, true) )
				MessageBox(hWnd, "Visual PinSPA installed successfully!", g_szCaption, MB_ICONINFORMATION|MB_OK);
			DisplayInstalledVersion(hWnd);
			EnabledButtons(hWnd);
			break;

		case IDC_UNREGISTER:
			if ( RegisterUnregisterVPinSPA(hWnd, false) )
				MessageBox(hWnd, "Visual PinSPA uninstalled successfully! \nYou may now safely delete your Visual PinSPA Directory!", g_szCaption, MB_ICONINFORMATION|MB_OK);
			DisplayInstalledVersion(hWnd);
			EnabledButtons(hWnd);
			break;
